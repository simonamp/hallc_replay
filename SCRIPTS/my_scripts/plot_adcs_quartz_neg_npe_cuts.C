...
// Int_t run_num = 393;
  ...
  for(Int_t i=0; i < nentries; i++){
	...
	if (ip == 3){
	  ...
	  for(Int_t adc1_ihit = 0; adc1_ihit < nadchits[ip][0]; adc1_ihit++)  {
	    ...
	    if (adc1bar == 6 && (track_y[0] + track_yp[0] * 276.35) > 15. && (track_y[0] + track_yp[0] * 276.35) < 24.){
	      Double_t adc1val = 0.;
	      Double_t adc1val = adc_values[ip][0][2][adc1_ihit] * 0.02;
	      histo7->Fill(adc1val);
	    } else if (adc1bar == 7 && (track_y[0] + track_yp[0] * 276.35) > 9. && (track_y[0] + track_yp[0] * 276.35) < 19.){
	      Double_t adc1val = 0.;
	      adc1val = adc_values[ip][0][2][adc1_ihit] * 0.02;
	      histo8->Fill(adc1val);
	    } else if (adc1bar == 8 && (track_y[0] + track_yp[0] * 276.35) > 5. && (track_y[0] + track_yp[0] * 276.35) < 14.){
	      Double_t adc1val = 0.;
	      adc1val = adc_values[ip][0][2][adc1_ihit] * 0.02;
	      histo9->Fill(adc1val);
	    } else if (adc1bar == 9 && (track_y[0] + track_yp[0] * 276.35) > 0. && (track_y[0] + track_yp[0] * 276.35) < 7.){
	      Double_t adc1val = 0.;
	      adc1val = adc_values[ip][0][2][adc1_ihit] * 0.02;
	      histo10->Fill(adc1val);
	    } else if (adc1bar == 10 && (track_y[0] + track_yp[0] * 276.35) > -4. && (track_y[0] + track_yp[0] * 276.35) < 4.){
	      Double_t adc1val = 0.;
	      adc1val = adc_values[ip][0][2][adc1_ihit] * 0.02;
	      histo11->Fill(adc1val);

	    } else if (adc1bar == 11 && (track_y[0] + track_yp[0] * 276.35) > -10. && (track_y[0] + track_yp[0] * 276.35) < -1.){
	      Double_t adc1val = 0.;
	      adc1val = adc_values[ip][0][2][adc1_ihit] * 0.02;
	      histo12->Fill(adc1val);

	    } else if (adc1bar == 12 && (track_y[0] + track_yp[0] * 276.35) > -15. && (track_y[0] + track_yp[0] * 276.35) < -6.){
	      Double_t adc1val = 0.;
	      adc1val = adc_values[ip][0][2][adc1_ihit] * 0.02;
	      histo13->Fill(adc1val);
	    } else if (adc1bar == 13 && (track_y[0] + track_yp[0] * 276.35) > -19. && (track_y[0] + track_yp[0] * 276.35) < -11.){
	      Double_t adc1val = 0.;
	      adc1val = adc_values[ip][0][2][adc1_ihit] * 0.02;
	      histo14->Fill(adc1val);
	    } else if (adc1bar == 14 && (track_y[0] + track_yp[0] * 276.35) > -24. && (track_y[0] + track_yp[0] * 276.35) < 17.){
	      Double_t adc1val = 0.;
	      adc1val = adc_values[ip][0][2][adc1_ihit] * 0.02;
  ...
  c3->Divide(3,3);
  c3->cd(1);
  //  formatit(histo7,60.,130.,"SHMS_S2Y_7Pos");
  formatit(histo7,30.,190.,"SHMS_S2Y_7Neg");
  c3->cd(2);
  //  formatit(histo8,80.,160.,"SHMS_S2Y_8Pos");
  formatit(histo8,70.,155.,"SHMS_S2Y_8Neg");
  c3->cd(3);
  //  formatit(histo9,40.,85,"SHMS_S2Y_9Pos");
  formatit(histo9,20.,130,"SHMS_S2Y_9Neg");
  c3->cd(4);
  //  formatit(histo10,60.,160.,"SHMS_S2Y_10Pos");
  formatit(histo10,22.,140.,"SHMS_S2Y_10Neg");
  c3->cd(5);
  //  formatit(histo11,58.,120.,"SHMS_S2Y_11Pos");   
  formatit(histo11,55.,160.,"SHMS_S2Y_11Neg");   
  c3->cd(6);
  //  formatit(histo12,40,90,"SHMS_S2Y_12Pos");
  formatit(histo12,55,165,"SHMS_S2Y_12Neg");
  c3->cd(7);
  //  formatit(histo13,40,100,"SHMS_S2Y_13Pos");
  formatit(histo13,22,88,"SHMS_S2Y_13Neg");
  c3->cd(8);
  //  formatit(histo14,40,95,"SHMS_S2Y_14Pos");
  formatit(histo14,40,210,"SHMS_S2Y_14Neg");
  c3->cd(9);
  formatit(histo15,40,120,"SHMS_S2Y_15Neg"); 

  c3->Print("488_fresh_quartz_pint_npe_neg_withc.png");
  c3->Print("488_fresh_replay_quartz_pint_npe_neg_withc.pdf");
