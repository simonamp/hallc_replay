...
int main(){
  ...
  for(Int_t i=0; i < nentries; i++){
	...
	if (ip == 3){
	  ...
	  for(Int_t adc1_ihit = 0; adc1_ihit < nadchits[ip][0]; adc1_ihit++)  {
	    ...
	    if (adc1bar == 6 && (track_x[0] + track_xp[0] * 276.35) < 0.){
	      //	    if (adc1bar == 6 && (track_x[0] + track_xp[0] * 276.35) > 0.){
	      Double_t adc1val = 0.;
	      Double_t adc1val = adc_values[ip][0][2][adc1_ihit] * 0.02;
	      histo7->Fill(adc1val);

	    } else if (adc1bar == 7 && (track_x[0] + track_xp[0] * 276.35) < 0.){
	      //	    } else if (adc1bar == 7 && (track_x[0] + track_xp[0] * 276.35) > 0.){
	      Double_t adc1val = 0.;
	      adc1val = adc_values[ip][0][2][adc1_ihit] * 0.02;
	      histo8->Fill(adc1val);

	    } else if (adc1bar == 8 && (track_x[0] + track_xp[0] * 276.35) < 0.){
	      //	    } else if (adc1bar == 8 && (track_x[0] + track_xp[0] * 276.35) > 0.){
	      Double_t adc1val = 0.;
	      adc1val = adc_values[ip][0][2][adc1_ihit] * 0.02;
	      histo9->Fill(adc1val);

	    } else if (adc1bar == 9 && (track_x[0] + track_xp[0] * 276.35) < 0.){
	      //	    } else if (adc1bar == 9 && (track_x[0] + track_xp[0] * 276.35) > 0.){
	      Double_t adc1val = 0.;
	      adc1val = adc_values[ip][0][2][adc1_ihit] * 0.02;
	      histo10->Fill(adc1val);

	    } else if (adc1bar == 10 && (track_x[0] + track_xp[0] * 276.35) < 0.){
	      //	    } else if (adc1bar == 10 && (track_x[0] + track_xp[0] * 276.35) > 0.){
	      Double_t adc1val = 0.;
	      adc1val = adc_values[ip][0][2][adc1_ihit] * 0.02;
	      histo11->Fill(adc1val);

	    } else if (adc1bar == 11 && (track_x[0] + track_xp[0] * 276.35) < 0.){
	      //	    } else if (adc1bar == 11 && (track_x[0] + track_xp[0] * 276.35) > 0.){
	      Double_t adc1val = 0.;
	      adc1val = adc_values[ip][0][2][adc1_ihit] * 0.02;
	      histo12->Fill(adc1val);

	    } else if (adc1bar == 12 && (track_x[0] + track_xp[0] * 276.35) < 0.){
	      //	    } else if (adc1bar == 12 && (track_x[0] + track_xp[0] * 276.35) > 0.){
	      Double_t adc1val = 0.;
	      adc1val = adc_values[ip][0][2][adc1_ihit] * 0.02;
	      histo13->Fill(adc1val);

	    } else if (adc1bar == 13 && (track_x[0] + track_xp[0] * 276.35) < 0.){
	      //	    } else if (adc1bar == 13 && (track_x[0] + track_xp[0] * 276.35) > 0.){
	      Double_t adc1val = 0.;
	      adc1val = adc_values[ip][0][2][adc1_ihit] * 0.02;
	      histo14->Fill(adc1val);

	    } else if (adc1bar == 14 && (track_x[0] + track_xp[0] * 276.35) < 0.){
	      //	    } else if (adc1bar == 14 && (track_x[0] + track_xp[0] * 276.35) > 0.){
	      Double_t adc1val = 0.;
	      adc1val = adc_values[ip][0][2][adc1_ihit] * 0.02;
  ...
  c3->cd(1);
  formatit(histo7,30.,190.,"SHMS_S2Y_7Neg"); // less than
  //formatit(histo7,7.,100.,"SHMS_S2Y_7Neg"); // more than

  c3->cd(2);
  formatit(histo8,70.,155.,"SHMS_S2Y_8Neg"); // less than
  //formatit(histo8,48.,140.,"SHMS_S2Y_8Neg"); // more than

  c3->cd(3);
  formatit(histo9,20.,140,"SHMS_S2Y_9Neg"); // less than
  //formatit(histo9,9.,90,"SHMS_S2Y_9Neg"); // more than

  c3->cd(4);
  formatit(histo10,25.,160.,"SHMS_S2Y_10Neg"); // less than
  //formatit(histo10,20.,128.,"SHMS_S2Y_10Neg"); // more than

  c3->cd(5);
  formatit(histo11,68.,180.,"SHMS_S2Y_11Neg"); // less than   
  //formatit(histo11,50.,154.,"SHMS_S2Y_11Neg"); // more than   

  c3->cd(6);
  formatit(histo12,70,170,"SHMS_S2Y_12Neg"); // less than
  //formatit(histo12,50,144,"SHMS_S2Y_12Neg"); // more than

  c3->cd(7);
  formatit(histo13,35,88,"SHMS_S2Y_13Neg"); // less than
  //formatit(histo13,23,70,"SHMS_S2Y_13Neg"); // more than

  c3->cd(8);
  formatit(histo14,100,210,"SHMS_S2Y_14Neg"); // less than
  //formatit(histo14,45,148,"SHMS_S2Y_14Neg"); // more than

  c3->cd(9);
  formatit(histo15,40,185,"SHMS_S2Y_15Neg"); // less than
  //formatit(histo15,15,105,"SHMS_S2Y_15Neg"); // more than

  c3->Print("488_fresh_quartz_pint_npe_neg_withc_xlt0_noneiny.png");
  c3->Print("488_fresh_replay_quartz_pint_npe_neg_withc_xlt0_noneiny.pdf");
