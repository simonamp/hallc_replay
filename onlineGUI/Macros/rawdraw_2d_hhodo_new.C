...
void UserScript()
{
  ...
  Int_t pair_pl[4]={1,0,3,2};
  Double_t dis_tdc_min[4]={-1300.,-1400.,-1500.,-1700.};
  Double_t dis_tdc_max[4]={-800.,-1000.,-900.,-800.};
  const TString opp_plane[4] = {";Y1 plane paddle number; Tdc (.1/chan)",";X1 plane paddle number;Tdc (.1/chan) ",";Y2 plane paddle number;Tdc (.1/chan) ",";X2 plane paddle number;Tdc (.1/chan) "};
  ...
  TH2F* hadcint_adcpeak[NPLANES*NSIDES*MAXBARS];
  ...
  for(UInt_t ip = 0; ip < NPLANES; ip++) {
	    for(UInt_t ibar = 0; ibar < nbars[ip]; ibar++) {
	      ...
	      TString h2dtname="uh2dtdc"+plane_names[ip]+i2dbarname;
              htdc_tdc[ip*MAXBARS+ibar]= new TH2F(h2dtname,h2dttitle,100,-1200,-500,100,-1200,-500.);
	      ...
	      htdc_dis[ip*MAXBARS+ibar][0]= new TH2F(h2dtname,h2dttitle,dis_bin[ip],1,dis_bin[ip]+1,50,dis_tdc_min[ip],dis_tdc_max[ip]);
	      ...
	      htdc_dis[ip*MAXBARS+ibar][1]= new TH2F(h2dtname,h2dttitle,dis_bin[ip],1,dis_bin[ip]+1,50,dis_tdc_min[ip],dis_tdc_max[ip]);
	    }
  }
  //
  //
  ...
  for(UInt_t ip = 0; ip < NPLANES; ip++) {
    for(UInt_t iside = 0; iside < NSIDES; iside++) {
	    for(UInt_t ibar = 0; ibar < nbars[ip]; ibar++) {
	      ...
	      TString h2daptitle= "h2dadcPadc"+plane_names[ip]+i2dbarname+sides[iside]+"; ADC peak; ADC int (pC)";
	      TString h2dapname="uh2dadcPadc"+plane_names[ip]+i2dbarname+sides[iside];
              if ( h2dindex < NPLANES*NSIDES*MAXBARS) {
              htdc_adc[h2dindex]= new TH2F(h2dname,h2dtitle,200,-1000,-500,50,0.,100.);
              hadc_atdc[h2dindex]= new TH2F(h2daname,h2datitle,50,0,200.,40,100,120.);
              hadcint_adcpeak[h2dindex]= new TH2F(h2dapname,h2daptitle,100,0,500.,50,0,100.);
  ...
  for(UInt_t iev = 0, N = T->GetEntries(); iev < N; iev++) {
    ...
    Double_t tdcbar_save=-1;
	  ...
	  //cout << iev << " " << ip << " " << ntdchits[ip][0]<< " " << ntdchits[ip][1] << endl;
		...
		for(Int_t tdc2_ihit = 0; tdc2_ihit < ntdchits[ip][1]; tdc2_ihit++)  {
 		UInt_t tdc2bar = TMath::Nint(tdc_paddles[ip][1][tdc2_ihit]) - 1;
		Double_t tdc2val = tdc_values[ip][1][0][tdc1_ihit];
		for(Int_t tdc3_ihit = 0; tdc3_ihit < ntdchits[pair_pl[ip]][0]; tdc3_ihit++)  {
 		UInt_t tdc3bar = TMath::Nint(tdc_paddles[pair_pl[ip]][0][tdc3_ihit]) - 1;
		for(Int_t tdc4_ihit = 0; tdc4_ihit < ntdchits[pair_pl[ip]][1]; tdc4_ihit++)  {
		  UInt_t tdc4bar = TMath::Nint(tdc_paddles[pair_pl[ip]][1][tdc4_ihit]) - 1;
	...
	//
		...
		for(Int_t adc3_ihit = 0; adc3_ihit < nadchits[ip][iside]; adc3_ihit++)  {
 		  UInt_t adcbar = TMath::Nint(adc_paddles[ip][iside][adc3_ihit]) - 1;
		  Double_t adcival = int_values[ip][iside][2][adc3_ihit]*adcbit_to_pC;
		  Double_t adcpval = amp_values[ip][iside][4][adc3_ihit]*.244*2;
		  Double_t adctval =ptraw_values[ip][iside][6][adc3_ihit]*0.062;
		  //    cout << ip << " " << iside << " " << adc3_ihit <<  " " << adcbar << " " << adcpval << " " << adctval << endl;
		  hadc_atdc[hindex_base+adcbar]->Fill(adcpval,adctval);
		  hadcint_adcpeak[hindex_base+adcbar]->Fill(adcpval,adcival);
		}
